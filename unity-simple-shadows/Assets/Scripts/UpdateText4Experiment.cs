...
public class UpdateText4Experiment : MonoBehaviour {
    ...
    public SetWorldAnchor4Experiment anchorManager; // consider changing this for consistency across scripts 
    ...
    {        
        string dString = string.Format("{0:N3}", anchorManager.GetSurfaceHeight());
        myText = "Surf Height | " + dString;
        UpdateMyText();
        UpdateMySlider((anchorManager.GetSurfaceHeight() + 0.1f) / 0.2f);
    ...
    public void UpdateTrialText4ShadowContrastExp(int trial, int subTrial)
    {
        myText = "[ Trial : " + trial + "/20\t\t"
            + "SubTrial : " + subTrial + "/16 ]"; 
        UpdateMyText();
    }
